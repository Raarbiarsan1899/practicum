---
title: "Bayesian Methods for CDH dataset using Birth and 2yrs"
...
Use expectation-maximization (EM) algorithm to impute missing values. Setting **8** underlying latent components.

```{r}
# estimate number of latent variables, run too long
# missMDA::estim_ncpFAMD(cdh_iq_pca, ncp.max = 10, threshold = 0.0001)
...
# $criterion
#        0        1        2        3        4        5        6        7        8 
# 2.412049 1.849291 1.729275 1.694678 1.661605 1.623954 1.621629 1.613953 1.600654 
#        9       10 
# 1.609584 1.612483

cdh_imputed_famd <- missMDA::imputeFAMD(cdh_iq_pca, ncp = 8)

# Doesn't work
# missMDA::MIFAMD(cdh_iq_pca, ncp = 5, threshold = 0.001)
...
**Does not need to normalization since the FAMD function would do it automatically**
...
# missMDA::estim_ncpFAMD(cdh_normalized, threshold = 0.001)
# factor analysis
...
plot_ly(kmeans_cdh,
  ...
  layout(scene = list(xaxis = list(title = 'NDD related',
  																 range = c(-20, 10)),
                     yaxis = list(title = 'CDH therapies',
                     						 range = c(-8, 6)),
                     zaxis = list(title = 'Body metrics',
                     						 range = c(-10, 5))))
