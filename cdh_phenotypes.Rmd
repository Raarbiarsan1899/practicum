...
tibble(
	...
	labs(x = "missing values",
			 ...
			 title = "Figure 1. missing values for variables collected at birth",
...
tibble(
	...
	labs(x = "missing values",
			 y = "counts of variables",
			 title = "Figure 2. missing values for variables collected at birth",
			 fill = "collected time point")
```
...
# imputation
cdh <- cdh %>% 
	replace_na(list(need_neo_card_surg = 2, gu_anomaly = 0, clp = 0,
...
```{r using_five_year_variable, eval = FALSE, echo = FALSE}
# data cleaning
cdh_iq_5yrs <- cdh %>%
	filter(!is.na(crf7_da_fsiq_composite)) %>% 
	# select only columns are at least 80% complete in original datasets
	# because only those columns are useful in predicting missing IQ values
	select(which(apply(cdh_iq_temp, 2,
										 function(x) sum(!is.na(x))) > nrow(cdh_iq_temp) * 0.8)) %>% 
	select(crf7_da_fsiq_composite, everything()) %>%
	# single value
	select(-dead, -osp_tracheostomy, -discharge, -repaired, -SampleID)

cdh_iq_5yrs %>% skimr::skim()
# df_status_tube, fedu, gender, hosp_2yr, medu, income,
# nitric_oxide_therapy, patch, ph1, prenatal_diagnosis,
# ther2yr_any, transfer_inborn

for (i in 1:ncol(cdh_iq_5yrs)) {
	cdh_iq_5yrs[which(is.na(cdh_iq_5yrs[, i])), i] <- median(cdh_iq_5yrs[[i]], na.rm = TRUE)
	if (length(unique(cdh_iq_5yrs[[i]])) <= 3) {
		cdh_iq_5yrs[[i]] <- as.factor(cdh_iq_5yrs[[i]])
	}
}

# write_csv(cdh_iq_5yrs, "./cdh_iq_5yrs.csv")
...
dl_epoch_birth %>% 
    ...
    title = "Figure 6. Gradient decendent modeling"
  )
...
dl_epoch_2yrs %>% 
    ...
    x = "Models",
    y = "Mean Squared Errors",
    title = "Figure 7. Gradient decendent modeling"
  )
...
dl %>% 
	...
	geom_density() +
	labs(
		title = "Figure 8. Distribution of the difference between predicted and true values"
	)
...
According to Figure 9, all the models at least out-performed null model (predict the IQ values with mean IQ). But all the models has higher out-sample error (MSPE) in comparison with training error (MSE). It is not confident whether the models would have some prediction values.
